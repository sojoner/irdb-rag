.PHONY: help build test test-coverage db-backup db-restore
...
db-backup:
	...
	@BACKUP_FILE="$(BACKUP_DIR)/rag_chat_$$(date +%Y%m%d_%H%M%S).dump" && \
		docker exec $(DB_CONTAINER) pg_dump -U rag_user -d rag_chat -F c -f /tmp/temp_backup.dump && \
		docker cp $(DB_CONTAINER):/tmp/temp_backup.dump $$BACKUP_FILE && \
		docker exec $(DB_CONTAINER) rm /tmp/temp_backup.dump && \
...
db-restore:
	...
	@docker exec $(DB_CONTAINER) psql -U rag_user -d postgres -c "SELECT pg_terminate_backend(pid) FROM pg_stat_activity WHERE datname='rag_chat' AND pid != pg_backend_pid();" 2>/dev/null || true
	@echo "Dropping and recreating database..."
	@docker exec $(DB_CONTAINER) psql -U rag_user -d postgres -c "DROP DATABASE IF EXISTS rag_chat; CREATE DATABASE rag_chat;"
	...
	@docker exec $(DB_CONTAINER) psql -U rag_user -d rag_chat -c "SELECT COUNT(*) as documents FROM documents;"
