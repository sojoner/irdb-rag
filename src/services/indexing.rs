...
pub async fn index_path(pool: &PgPool, embedder: &Embedder, path: &str) -> Result<Vec<uuid::Uuid>> {
    index_path_with_config(pool, embedder, path, None, false).await
}
...
pub async fn index_path_with_config(
        ...
        for (batch_idx, batch) in files.chunks(batch_size).enumerate() {
            ...
            let futures: Vec<_> = batch
                ...
                .map(|(idx_in_batch, entry)| {
                    ...
                    async move { index_file(&pool, &embedder, &file_path, settings, skip_embedding).await }
                })
                .collect();
    ...
    } else if let Some(id) = index_file(pool, embedder, path, settings, skip_embedding).await? {
        indexed_ids.push(id);
    }
...
async fn index_file(
    ...
    // Stage 4: Generate Embeddings (optional - can be skipped for BM25-only mode)
    let all_embeddings = if skip_embedding {
        let start_stage4 = Instant::now();
        tracing::info!("  ├─ Stage 4/5: Skipping embeddings (BM25-only mode)...");
        let stage4_duration = start_stage4.elapsed();
        tracing::info!(
            "  │   ⏭️ Skipped: {:.2}s | Will generate embeddings asynchronously",
            stage4_duration.as_secs_f64()
        );
        
        // Generate empty embeddings for document and chunks (will be filled later by background worker)
        let mut empty_embeddings = Vec::new();
        empty_embeddings.push(Vec::new()); // Empty embedding for document
        for _ in 0..enriched_chunks.len() {
            empty_embeddings.push(Vec::new()); // Empty embeddings for each chunk
        }
        empty_embeddings
    } else {
        let start_stage4 = Instant::now();
    ...
    // Insert document with appropriate embedding status
    let embedding_status_str = if skip_embedding { "pending" } else { "completed" };
    let doc_id = db::insert_document(
        ...
        db::InsertDocumentParams {
            ...
            content_hash,
            embedding_status: Some(embedding_status_str),
        },
    )
    .await?;
    ...
    for (idx, raw_chunk) in raw_chunks.iter().enumerate() {
        let chunk_embedding = all_embeddings[idx + 1].clone();
        chunk_params.push(db::InsertChunkParams {
            chunk_index: idx as i32,
            content: raw_chunk.clone(),
            embedding: chunk_embedding,
            page_number: None,
            embedding_status: Some(embedding_status_str.to_string()),
        });
    }
...
pub async fn index_url(
    ...
    index_url_with_config(pool, embedder, url, None, false).await
}
...
pub async fn index_url_with_config(
    ...
    // Embed & Store (optionally skip embedding for BM25-only mode)
    let all_embeddings = if skip_embedding {
        // Generate empty embeddings for document and chunks (will be filled later by background worker)
        let mut empty_embeddings = Vec::new();
        empty_embeddings.push(Vec::new()); // Empty embedding for document
                ...
                if matches!(event.kind, EventKind::Create(_) | EventKind::Modify(_)) {
                    for path in event.paths {
                        if path.is_file() {
                            ...
                            if let Err(e) = index_file(pool, embedder, &path, None, false).await {
