    ...
    let do_send = move || {
        ...
        #[cfg(target_arch = "wasm32")]
        {
            ...
            leptos::task::spawn_local(async move {
                ...
                let mut search_opts = web_sys::RequestInit::new();
                ...
                leptos::logging::log!("Chat: Sending chat request with {} docs", doc_ids.as_ref().map(|d: &Vec<Uuid>| d.len()).unwrap_or(0));
...
#[cfg(target_arch = "wasm32")]
async fn fetch_conversation_messages(conversation_id: Uuid) -> Result<Vec<ChatMessage>, String> {
    use wasm_bindgen::JsCast;
    use wasm_bindgen_futures::JsFuture;
...
#[cfg(not(target_arch = "wasm32"))]
async fn fetch_conversation_messages(_conversation_id: Uuid) -> Result<Vec<ChatMessage>, String> {
    Err("Client-side only".to_string())
}
