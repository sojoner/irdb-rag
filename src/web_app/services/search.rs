...
pub async fn search_documents(request: SearchRequest) -> Result<SearchResponse, ServerFnError> {
    ...
    let has_filters = request.category_id.is_some()
        || (request.keywords.as_ref().is_some_and(|k| !k.is_empty()))
        || (request.concepts.as_ref().is_some_and(|c| !c.is_empty()))
        || (request.locations.as_ref().is_some_and(|l| !l.is_empty()))
        || (request.persons.as_ref().is_some_and(|p| !p.is_empty()))
        || (request.organizations.as_ref().is_some_and(|o| !o.is_empty()))
        || (request.authors.as_ref().is_some_and(|a| !a.is_empty()));
