...
pub async fn get_documents_by_ids(pool: &PgPool, ids: &[Uuid]) -> Result<Vec<Document>> {
    ...
    let docs = sqlx::query_as::<_, Document>(
        ...
        SELECT id, title, content, source_path, source_type, summary, author,
               ...
               entities, metadata, embedding, content_hash, embedding_status
        FROM documents
        WHERE id = ANY($1)
        "#,
    )
...
pub struct InsertDocumentParams<'a> {
    ...
    pub embedding_status: Option<&'a str>,
}
...
pub async fn insert_document(pool: &PgPool, params: InsertDocumentParams<'_>) -> Result<Uuid> {
    ...
    let dims = get_embedding_dimensions()?;
    let embedding_status = params.embedding_status.unwrap_or("pending");
    
    let sql = format!(
        r#"
        INSERT INTO documents (
            title, content, source_path, source_type, embedding,
            summary, keywords, locations, entities, author, category_id, metadata, content_hash, status, embedding_status
        )
        VALUES ($1, $2, $3, $4, $5::vector({}), $6, $7, $8, $9, $10, $11, $12, $13, 'indexed', $14::embedding_status)
        RETURNING id
        "#,
        dims
    );
...
pub async fn insert_chunk(
    ...
    let embedding_str = embedding_to_string(embedding);
    let status = embedding_status.unwrap_or("pending");
    ...
    let sql = format!(
        r#"
        INSERT INTO document_chunks (document_id, chunk_index, content, embedding, page_number, token_count, embedding_status)
        VALUES ($1, $2, $3, $4::vector({}), $5, $6, $7::embedding_status)
        RETURNING id
        "#,
        dims
    );
...
pub async fn insert_chunks_batch(
    ...
    let mut embedding_statuses = Vec::with_capacity(chunks.len());
    ...
    for chunk in chunks {
        ...
        embedding_statuses.push(chunk.embedding_status.unwrap_or_else(|| "pending".to_string()));
    }
    ...
    let sql = format!(
        r#"
        INSERT INTO document_chunks (document_id, chunk_index, content, embedding, page_number, token_count, embedding_status)
        SELECT $1, t.chunk_index, t.content, t.embedding::vector({}), t.page_number, t.token_count, t.embedding_status::embedding_status
        FROM UNNEST($2::INTEGER[], $3::TEXT[], $4::TEXT[], $5::INTEGER[], $6::INTEGER[], $7::TEXT[]) 
        AS t(chunk_index, content, embedding, page_number, token_count, embedding_status)
        "#,
        dims
    );
...
fn row_to_document(r: &sqlx::postgres::PgRow) -> Document {
    ...
    Document {
        ...
        embedding: None, // Embeddings are not fetched to avoid pgvector deserialization issues
...
pub async fn search_and_get_documents(
    ...
    let rows = sqlx::query(
        ...
        SELECT id, title, content, source_path, source_type, summary, author,
               category_id, keywords, locations, created_at, status,
               entities, metadata, content_hash, embedding_status
        FROM documents
        WHERE id = ANY($1)
    ...
    let documents: Vec<Document> = rows.iter().map(row_to_document).collect();
...
mod tests {
    ...
    fn create_test_document(id: Uuid, author: Option<&str>, entities: Option<Value>) -> Document {
        use chrono::Utc;
        use crate::domain::models::EmbeddingStatus;
        Document {
            id,
            ...
            created_at: Utc::now(),
            embedding: None,
            embedding_status: EmbeddingStatus::default(),
        }
    }
...
/// Ensure a conversation exists (create if not exists)
pub async fn ensure_conversation_exists(pool: &PgPool, id: Uuid, title: &str) -> Result<()> {
    sqlx::query(
        r#"
        INSERT INTO conversations (id, title, created_at, updated_at)
        VALUES ($1, $2, NOW(), NOW())
    ...
    tracing::debug!("Ensured conversation exists: {} with title: {}", id, title);
    Ok(())
}
...
pub async fn batch_skip_unenriched_documents(pool: &PgPool) -> Result<u64> {
    ...
    tracing::info!(
        "✓ Batch skipped {} unenriched documents",
        skipped
    );
