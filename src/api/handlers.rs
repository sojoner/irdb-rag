...
> {
    tracing::info!(
        "Received streaming conversation chat request: {} messages, context_chunks={}, document_ids={:?}, dual_agents={}",
        req.messages.len(),
        req.context_chunks,
        req.document_ids,
        req.dual_agents
    ...
    let stream = async_stream::stream! {
        if req.dual_agents {
            ...
            if let Ok(mut moderator_stream) = moderator_result {
                while let Some(result) = moderator_stream.next().await {
                    match result {
                        Ok(chunk) if !chunk.is_empty() => {
                            moderator_response.push_str(&chunk);
                        ...
                        Err(e) => {
                            tracing::error!("Moderator stream error: {}", e);
            ...
            if let Ok((mut rag_stream, sources)) = rag_reporter_result {
                rag_sources = sources;
                while let Some(result) = rag_stream.next().await {
                    match result {
                        Ok(chunk) if !chunk.is_empty() => {
                            rag_response.push_str(&chunk);
                            let json = serde_json::json!({
                                "agent": "rag_reporter",
                                "type": "chunk",
                                "content": chunk
                        ...
                        Err(e) => {
                            tracing::error!("RAG Reporter stream error: {}", e);
                            let error = serde_json::json!({
                                "agent": "rag_reporter",
                                "type": "error",
                                "message": e.to_string(),
            ...
            if !moderator_response.is_empty() || !rag_response.is_empty() {
                if let Err(e) = db::save_message(&pool, conversation_id, "user", &last_user_message_clone).await {
                    tracing::error!("Failed to save user message: {}", e);
                }

                if !moderator_response.is_empty() {
                    if let Err(e) = db::save_message(&pool, conversation_id, "moderator", &moderator_response).await {
                        tracing::error!("Failed to save moderator response: {}", e);
                    }
                }

                if !rag_response.is_empty() {
                    if let Err(e) = db::save_message(&pool, conversation_id, "rag_reporter", &rag_response).await {
                        tracing::error!("Failed to save RAG reporter response: {}", e);
                    }
                }
            }
            ...
            tracing::info!("Dual-agent stream completed successfully for conversation_id={}", conversation_id);
        } else {
